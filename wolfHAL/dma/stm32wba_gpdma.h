 ...
 */
typedef struct {
    whal_Stm32wbaGpdma_Dir dir;
    size_t srcAddr;
    size_t dstAddr;
    size_t nbytes;                   /* Byte count (max 65535) */
    whal_Stm32wbaGpdma_Width srcWidth;
    whal_Stm32wbaGpdma_Width dstWidth;
    whal_Stm32wbaGpdma_Inc srcInc;
    whal_Stm32wbaGpdma_Inc dstInc;
    uint8_t reqSel;                  /* REQSEL[5:0] (ignored for MEM_TO_MEM) */
} whal_Stm32wbaGpdma_ChCfg;
...
} whal_Stm32wbaGpdma_Cfg;

extern const whal_DmaDriver whal_Stm32wbaGpdma_Driver;

/*
 * @brief DMA completion callback invoked from the channel IRQ handler.
 */
typedef void (*whal_Stm32wbaGpdma_Callback)(void *ctx, whal_Error err);
