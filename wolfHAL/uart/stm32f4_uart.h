...
/*
 * @brief Start an asynchronous UART transmit.
 *
 * @param uartDev UART device instance.
 * @param data    Buffer to transmit.
 * @param dataSz  Number of bytes to transmit.
 *
 * @retval WHAL_ENOTSUP Polled driver does not implement async; provided so
 *                      stm32l1_uart can alias the symbol.
 * @retval WHAL_EINVAL  Invalid arguments.
 */
whal_Error whal_Stm32f4_Uart_SendAsync(whal_Uart *uartDev, const void *data, size_t dataSz);

/*
 * @brief Start an asynchronous UART receive.
 *
 * @param uartDev UART device instance.
 * @param data    Receive buffer.
 * @param dataSz  Number of bytes to receive.
