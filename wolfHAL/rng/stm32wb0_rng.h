 ...
 *
 * Enables the RNG analog block (clears RNG_DIS), then probes RNGCLK via
 * TST_CLK and waits for REVCLK to confirm the clock is present. After
 * Init the device is left enabled so subsequent Generate calls do not
 * pay the oscillator startup cost.
 *
 * @param rngDev RNG device instance.
 *
 * @retval WHAL_SUCCESS Initialization completed.
 * @retval WHAL_ETIMEOUT RNGCLK did not appear after TST_CLK.
 */
 ...
 * @retval WHAL_ETIMEOUT Generation timed out waiting for RNGRDY.
 * @retval WHAL_EHARDWARE FAULT bit was set during generation.
 */
