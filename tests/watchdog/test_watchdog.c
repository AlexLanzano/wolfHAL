...
static void Test_Watchdog(void)
{
    ...
    if (state.magic != WDG_TEST_MAGIC) {
        ...
        WHAL_ASSERT_EQ(whal_Flash_Erase(&g_whalFlash, BOARD_FLASH_TEST_ADDR,
                                         BOARD_FLASH_SECTOR_SZ), WHAL_SUCCESS);
        do {
            err = whal_Flash_Write(&g_whalFlash, BOARD_FLASH_TEST_ADDR,
                                    &state, sizeof(state));
        } while (err == WHAL_ENOTREADY);
        WHAL_ASSERT_EQ(err, WHAL_SUCCESS);

        WHAL_ASSERT_EQ(whal_Watchdog_Init(&g_whalWatchdog), WHAL_SUCCESS);

        /* Verify refresh keeps us alive */
        for (int i = 0; i < 1000; i++) {
            Board_WaitMs(1);
            WHAL_ASSERT_EQ(whal_Watchdog_Refresh(&g_whalWatchdog),
                           WHAL_SUCCESS);
        }
